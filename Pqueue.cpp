...
/*  NOTE: This code is essentially word for word grabbed from a GIT Hub Source
 *
 *      Source: https://github.com/Ten0/updatable_priority_queue
 *
 *      This was not intentionally plagarized, the fact that we weren't able to at
 *      least refactor it is due to the timeline on which we discovered the need for
 *      it. Due to an error on our teams part we ended up having to scrap the original
 *      pQueue Code since we realized quite literally last minute that no built-in
 *      function existed to update the priority queue. This mistake arose from a
 *      lack of co-ordination that we now realize is far more crucial.
 *
 *      I (Adam) chose to keep this code instead of the original code since this code
 *      is BEAUTIFULLY written, even if it is somewhat obscure about how its actually
 *      working. Additionally the code does a fantastic job of integrating with existing code.
 *      Essentially I decided to keep it because I was immediately drawn to the original authors work.
 *
 *
 *
 * */
...
namespace improved_pQueue {
    ...
    /** Key has to be an u-int value (convertible to size_t)
    ...
    protected:
        std::vector<size_t> id_to_heappos;  //<- Not entirely sure what this is doing (ACS)
        // I assume it is used to manage priorities?
    ...
    public:
        ...
        void pop(bool remember_pop=false) {
            if(size() == 0)
                return; //Break if empty (ACS)
            id_to_heappos[heap.front().key] = -1-remember_pop;  //<-I do not know what hes doing here, I think its
            // changing something with the key value? (ACS)
            if(size() > 1)
